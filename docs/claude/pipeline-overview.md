...
| Уровень | Что | Запуск |
|---|---|---|
| Per-setId (21 hard-блок) | Полный набор проверок одного сета | `bash scripts/validate/validate_all.sh <setId>` |
...
**21 валидатор внутри validate_all.sh** перечислены в самом скрипте (`scripts/validate/validate_all.sh`).
