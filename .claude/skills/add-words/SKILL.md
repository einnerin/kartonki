...
### 1a. ОБЯЗАТЕЛЬНО — проверка темы на синонимы среди существующих

Перед созданием набора с новым `topic`:
...
Просмотри список глазами. **Не плодить дубль-темы**:
- Близкие написания: `Хай-тек` ↔ `Хайтек`, `Кофейня и работа баристой` ↔ `Кофе и работа баристой`
- Расширения: `Маркетинг` ↔ `Маркетинг и реклама`, `Здоровье` ↔ `Здоровье и фитнес`
- Синонимы / переформулировки: `Религия` ↔ `Религия и праздники`, `Хайтек` ↔ `IT`
- Заимствования и кальки: `Кибербезопасность` ↔ `IT-безопасность` ↔ `Information Security`

**Старая тема всегда выигрывает** над «улучшенным» новым именем. Если хочется переименовать — это отдельная осознанная задача со слиянием. См. `~/.claude/.../memory/feedback_no_duplicate_topics.md`.

Защита: `find_real_dupes.py` → `check_topic_synonyms` warning'ит при близких именах (нормализация: lowercase, без дефисов/пробелов/ё).
...
- **COMMON** — A1, детская лексика
- **UNCOMMON** — A2–B1
- **RARE** — B2
- **EPIC** — C1
- **LEGENDARY** — C2+ / архаика

**Правило: в одном наборе — максимум 2 смежных уровня редкости.** Набор привязан к уровню сложности (`level` поля `WordSetEntity` 1-5), и все 25 слов должны укладываться в 2 смежных уровня по шкале rarity (валидатор `validate_rarity_spread.py` это enforce'ит).

Типовые пары по уровню сета:
| level | Типичный разброс редкости |
|-------|---------------------------|
| 1 (основы) | COMMON + UNCOMMON |
| 2 (продвинутый) | UNCOMMON + RARE |
| 3 (углублённый) | RARE + EPIC |
| 4 (профессиональный) | EPIC + LEGENDARY |
| 5 (носитель) | LEGENDARY (+ единичный EPIC если нужен разброс) |

**Тема покрывает все 5 уровней через РАЗНЫЕ сеты**, а не через один сет. Минимум 5 наборов на тему (по одному на уровень).
...
WordEntity(
    ...
    transliteration = "[dɔg]",      // только en-ru: IPA в квадратных скобках. Для иврита НЕ заполнять (null)
),
```

**Требования:**
- Ровно **25 слов**, номер позиции 1..25
- Для английского — всегда заполнять `transliteration` (IPA в квадратных скобках, напр. `[dɔg]`)
- Для иврита — `transliteration` НЕ используется, оставить `null`: огласовка-никуд в `original` + озвучка TTS дают произношение (убрано 2026-05-28)

**Качество текстов (обязательно по стандартам — см. п.0):**
- `definition` — живой стиль учителя, нет однокоренных с `original`/`translation`, ≤16 слов / ≤90 символов
- `example` — живая речь носителя, целевое слово присутствует, пропуск однозначен, ≤14 слов / ≤90 символов (англ.) или ≤12 / ≤80 (иврит)
...
Запускает **все 8 валидаторов** (fields_filled, pos_values, group_sizes, text_lengths, rarity_spread, no_duplicates, no_cognates, blank_ambiguity).
...
| `validate_text_lengths` | Сократить определение (≤16 слов/90 симв) или пример (≤14/90 для англ., ≤12/80 для иврита) |
...
- [ ] Для английского — есть `transliteration` (IPA) у всех слов
- [ ] Для иврита — `transliteration` НЕ заполняется (null)
