...
- `semanticGroup` — латиницей, snake_case
...
## 7. Проверка после написания (ОБЯЗАТЕЛЬНО)

### 7.1. Ровно 25 слов
```bash
grep -c "setId = SET_ID," app/src/main/java/com/example/kartonki/data/WordDataXxx.kt
```
Результат должен быть `25`.

### 7.2. Уникальность внутри набора
```bash
grep "setId = SET_ID," app/src/main/java/com/example/kartonki/data/WordDataXxx.kt -A 1 | grep "original" | sort | uniq -d
```
Должно быть пусто.

### 7.3. Полировка QA (обязательно)

После написания набора прогнать `definition-polisher` и `example-polisher` на новых `setId`. Если правок много — значит стиль изначально не дотягивал, учти в следующий раз и приближайся к стандарту сразу при написании.

### 7.4. Полный аудит (рекомендуется после batch-изменений)
```bash
...
```
Ложные тревоги: `setId=0` (5 слов — награды достижений).
