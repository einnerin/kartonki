...
### 7.1a. Сверка огласовки с Dicta Nakdan (ТОЛЬКО he-ru)

**Зачем.** У нас нет билингва-тестера (иврит+русский), и огласовка (никуд) — самый частый класс ошибок: `יָחוּר` вместо `יִחוּר`, `תַּפְרַחַת` вместо `תִּפְרַחַת`, шва вместо камаца и т.п. Глазами их не поймать. Поэтому для he-ru огласовка сверяется с внешним эталоном — академическим накдан-движком Dicta (`nakdan.dicta.org.il`).

```bash
python scripts/validate/he_niqqud_check.py <newSetId>
```

Скрипт для каждого `original`: снимает никуд → берёт «голые» согласные → спрашивает у Накдана варианты огласовки → сверяет нашу (NFC-нормализованную) форму. Если нашей формы нет среди вариантов Накдана — слово помечается как кандидат на ошибку, с предложением эталона. Результаты кешируются в `scripts/validate/_nakdan_cache.json` (повторный прогон — без сети).

**Как читать вывод — это НЕ хард-гейт, а кандидаты:**
- **Одиночные существительные без пометки** `[loan?]` — Накдан почти всегда прав. Если он предлагает другую огласовку — **исправить на форму Накдана** (это ground-truth, не выдумка LLM).
- **`[loan?]` (заимствования)** — `אֶרוֹזְיָה`, `סוּקוּלֶנְט`, латиница-в-иврите: у Накдана нестабильно, обе формы приемлемы. **Игнорировать**, если наша форма читается.
- **Конструкт (смихут) и устойчивые/библейские фразы** — `לְשׁוֹן הָרַע`, `יַד הַגּוֹרָל`: Накдан анализирует слово в изоляции и даёт абсолютную форму вместо сопряжённой → **ложные срабатывания**. Сверяться с `python scripts/validate/he_niqqud_context.py <setId>` (подставляет слово в `example` — контекст снимает омонимию) или решать по смыслу.
- **Омонимы** — `כִּסּוּחַ` (покос) vs `כָּסוּחַ` (скошенный), `גִּנּוּן` (озеленение) vs `גַּנּוֹן` (детсад): Накдан в изоляции может выбрать не то чтение. Контекст-скрипт разрешает.

**Деградация без сети.** Если Dicta недоступна (таймаут/ошибка) — скрипт это печатает и пропускает слово; **генерацию набора не блокировать**, отметить «никуд не сверён, проверить позже» и продолжить. Сетевой сбой не должен останавливать pipeline.

**Бонус — детектор не-слов (консонантные ошибки).** Тот же скрипт печатает `⚠ NON-WORD`, если согласные `original` отсутствуют в лексиконе Dicta (`fnotfromwl=true`). Это ловит ошибки, которые огласовка-сверка пропускает: `פּוֹרִיחָה` вместо `פְּרִיחָה` (лишний `ו` — не слово). **Внимание: высокий шум на заимствованиях** — `טַבְלֶט`, `בּוֹיְלֶר`, `פֶּנָלְטִי`, `מוֹד` тоже «не из лексикона», но это нормальные ивритские заимствования, не ошибки. Реальная ошибка = слово ВЫГЛЯДИТ исконно-ивритским, но его нет в лексиконе, а рядом по написанию есть настоящее слово с тем же смыслом. Заимствования отсеивать глазами/по русскому переводу.

**Что чинить.** Только подтверждённые реальные ошибки: (а) огласовка — заменой на форму-эталон Накдана; (б) не-слова — заменой на исконное слово, подтверждённое в лексиконе Dicta. Спорные (конструкт/идиома/заимствование/семантика) — оставить либо вынести на ручное решение носителя. При правке `original` не забыть про FILL_IN_BLANK freshness (см. 7.5) — правка никуда в `example` инвалидирует хеш.
