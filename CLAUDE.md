...
1. **Обязательно проверь каждое новое слово на дубли** перед тем как вписать его в файл.
...
3. Соблюдай шкалу редкости: COMMON=A1, UNCOMMON=A2-B1, RARE=B2, EPIC=C1, LEGENDARY=C2+.
...
7. **Каждый набор должен содержать ровно 25 слов.** После написания каждого набора — обязательно подсчитай слова командой:
   ```bash
